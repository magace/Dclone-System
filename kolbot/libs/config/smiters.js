...
function LoadConfig() {
  ...
  Scripts.IPHunter = false; //  I always run it to log info to d2soj.com
  Scripts.DcloneKiller = true; // ALL WE NEED
	...
	*/
	...
	Config.Leader = "negligentuncle"; // Leader's ingame character name. Leave blank to try auto-detection (works in AutoBaal, Wakka, MFHelper)
	Config.QuitList = ["negligentuncle"]; // List of character names to quit with. Example: Config.QuitList = ["MySorc", "MyDin"];
  ...
  Scripts.PotCheck = false;
	...
	Scripts.OrgTorch = false;
		...
		Config.OrgTorch.GetFade = true; // Get fade by standing in a fire. You MUST have Last Wish, Treachery, or SpiritWard on your character being worn.
	...
	Config.TownHP = 20; // Go to town if life is under designated percent.
	...
	// Potion settings
	Config.UseHP = 86; // Drink a healing potion if life is under designated percent.
	...
	Config.AttackSkill[0] = sdk.skills.Smite; // Preattack skill.
	Config.AttackSkill[1] = sdk.skills.Smite; // Primary skill to bosses.
	Config.AttackSkill[2] = sdk.skills.Fanaticism; // Primary aura to bosses
	Config.AttackSkill[3] = sdk.skills.Smite; // Primary skill to others.
	Config.AttackSkill[4] = sdk.skills.Fanaticism; // Primary aura to others.
	...
	Config.AutoMule.Exclude = ["[name] == keyofhate", "[name] == keyofterror", "[name] == keyofdestruction","[name] == diablo'shorn","[name] == mephisto'sbrain","[name] == baal'seye"];
//
	// ############################### //
